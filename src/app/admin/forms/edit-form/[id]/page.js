...
const Page = () => {
  ...
  const [{ selectedFont, text, editTemplate }, dispatch] = useStateValue();
  ...
  useEffect(() => {
    dispatch({ type: "SET_FONT", selectedFont: editTemplate?.text_font });
    dispatch({ type: "SET_TEXT", text: editTemplate?.text_placeholder });
    setIsBold(editTemplate?.is_italic);
    setIsItalic(editTemplate?.is_bold);
    setFontSize(editTemplate?.text_size);
    setTextPosition({
      x: editTemplate?.text_position?.x,
      y: editTemplate?.text_position?.y,
    });
    setImageSrc(editTemplate?.bg_img);
    setImageUrl(editTemplate?.logo_img);
    setCanvasWidth(editTemplate?.canvas_width);
    setCanvasHeight(editTemplate?.canvas_height);
    setCompanyName(editTemplate?.company_name);
    setSelectedColor(editTemplate?.text_color);
    setBgColor(editTemplate?.bg_color);
    setBtnColor(editTemplate?.download_btn_color);
  }, []);
